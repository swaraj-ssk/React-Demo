        ...
        <div style={{ color: props.mode === 'dark' ? "white" : '#2b3035' }}>
            <div className="container my-3" >
                ...
                <div>
                    ...
                    <button className="btn btn-primary mx-1 my-1" onClick={toCopy}>Copy to Clipboard</button>
                    <button className="btn btn-primary mx-1 my-1" onClick={toClear}>Clear Text</button>
                    <button className="btn btn-primary mx-1 my-1" onClick={removeWhiteSpaces}>Remove Extra Spaces</button>
                </div>
            </div>
            <div className="container my-3">
                <h1>Your text summary </h1>
                <p>{text.length} characters and {text.split(/\s+/).filter(i => i !== "").length} number of words</p>
                <p>Time to read : {0.08 * text.split(/\s+/).filter(i => i !== "").length} minutes</p>
                <h1>Preview</h1>
                <p>{text}</p>
            </div>
        </div>
